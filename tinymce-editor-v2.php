...
class TinyMCEEditorPluginV2 extends Plugin {
	...
	public function onTwigTemplatePaths() {
		if(file_exists(__DIR__ . "/../../data/tinymce-editor-v2/templates")) {
			$this->grav["twig"]->twig_paths[] = __DIR__ . "/../../data/tinymce-editor-v2/templates";
		} else {
	...
	public function onTwigSiteVariables() {
		if($this->isAdmin()) {
			...
			$version = "tinymce";
			if($this->config["plugins"]["tinymce-editor-v2"]["apikey"] == "") {
				if(file_exists(__DIR__ . "/../../data/tinymce-editor-v2/js/" . $version)) {
					$this->grav["assets"]->add("user://data/tinymce-editor-v2/js/" . $version . "/tinymce.min.js");
				} else {
					$this->grav["assets"]->add("plugin://tinymce-editor-v2/js/" . $version . "/tinymce.min.js");
				}
			} else {
				$this->grav["assets"]->add("https://cloud.tinymce.com/4/tinymce.min.js?apiKey=" . urlencode($this->config["plugins"]["tinymce-editor-v2"]["apikey"]));
			}
			if(file_exists(__DIR__ . "/../../data/tinymce-editor-v2/css")) {
				$this->grav["assets"]->add("user://data/tinymce-editor-v2/css/editor.css");
			} else {
				$this->grav["assets"]->add("plugin://tinymce-editor-v2/css/editor.css");
			}
			if($this->config["plugins"]["tinymce-editor-v2"]["restrictions"]["blacklist"]) {
				foreach($this->grav["pages"]->root()->collection($this->config["plugins"]["tinymce-editor-v2"]["blacklist"], false) as $item) {
					if($item == $page) {
						return;
					}
				}
			...
			$whitelisted = true;
			if($this->config["plugins"]["tinymce-editor-v2"]["restrictions"]["whitelist"]) {
				$whitelisted = false;
				foreach($this->grav["pages"]->root()->collection($this->config["plugins"]["tinymce-editor-v2"]["whitelist"], false) as $item) {
					if($item == $page) {
						$whitelisted = true;
						break;
					}
			...
			if($whitelisted) {
				if(file_exists(__DIR__ . "/../../data/tinymce-editor-v2/blueprints")) {
					$this->grav["locator"]->addPath("blueprints", "", __DIR__ . "/../../data/tinymce-editor-v2/blueprints");
				} else {
